...
| CSRF | `SameSite=Lax` + an origin check on **every** state-changing request except the two service-token routes, which are named in `server/middleware/origin.ts` rather than inferred from a path prefix or a `Bearer` header; auth flows are POSTs from same-site forms. |
| Malicious insider / stale admin | `auth:ADMIN` limited to two people; all admin actions audit-logged; role revocation propagates ≤15 min; annual handover rotates everything. With only two holders, the last-admin guard counts **usable** holders only: disabling an account leaves its grant row in place, and a grant nobody can sign in to use would otherwise let the remaining admin drop their own and lock everyone out. |
...
- **Tokens** (verify/reset): `randomBytes(32)` hex, single-use, TTLs 24 h / 1 h / 24 h (admin), outstanding verifications and resets invalidated on reissue. A verification token also records the address it was mailed to and is refused once `users.email` differs, so re-pointing the row at somebody else's address cannot be confirmed with a link mailed to the old one.
- **Workspace addresses hold no password**: enforced by `assertPasswordAllowed` at every site that writes `users.password`, writes `users.email`, or mints a set-password token (reset redemption, self-service password change, admin reset, admin create, the merge credential fill, and both email-change routes), not only at the login entry points. The email-change routes are in that list because a Workspace address is proven by signing in with Google, never by typing it into a profile field: an account that could pre-claim an unused `@newtheatre.org.uk` address would be the account the real holder's first Google sign-in attaches to. A self-service email change also bumps `session_epoch`, so no other session survives the move.
