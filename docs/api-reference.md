...
| `PUT /api/users/:id/pending-google` | Set/clear `pending_google_email`: admin-directed link: the next Google sign-in with that address attaches to this account. Validated `@newtheatre.org.uk`; refuses any address another account already holds as its `email` (linked to Google or not) or already has pending. Whether the other account has a `google_sub` is beside the point: a pending marker beats an address match at sign-in, so a marker aimed at somebody else's address would attach their Google identity to this row and seal them into it. `pending_google_email` is unique, so two admins cannot point one address at two accounts |
...
Session + `auth:ADMIN`, mutations **[AUD]**: `GET /api/apps` (each with `hasToken`, false where no `service_tokens` row shares the name: such an app cannot be called at all), `POST /api/apps { name, namespace, displayName, baseUrl, hooksEnabled, manifestEnabled }` (409 on a duplicate name or namespace; links an already-issued token of the same name; both switches default to false), `PUT /api/apps/:id { displayName, baseUrl, hooksEnabled, manifestEnabled }` (name and namespace immutable; **every field is required**, because the update is a full replace and a defaulted switch would silently turn something off), `DELETE /api/apps/:id` (**withdraws every live role definition in the app's namespace and revokes the app's service tokens**, then removes the row, all in one batch: `role_definitions.app_id` has no foreign key to cascade, so a surviving definition would leave a dead role grantable and invisible to the suspect-grant report, and `requireServiceToken` never consults `app_id`, so an orphaned token would keep authenticating a decommissioned app inbound). Grants are untouched, as at any other withdrawal.

`manifestEnabled` is the switch for reading the app's role manifest, off by default ([ADR-0018](decisions/0018-manifest-declared-roles.md)). With it off, `POST /api/apps/:id/sync` answers "Manifest sync is disabled for this app" and the daily sweep skips the app; since [ADR-0024](decisions/0024-role-definitions-come-only-from-manifests.md) leaves no write route for role definitions, none of that app's new roles can then appear by any route at all. Both `app.registered` and `app.updated` record it, so the value it was flipped from is recoverable.
