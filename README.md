...
| [CLAUDE.md](CLAUDE.md) | you want the invariants and conventions in one place |
...
Small estate, small team: work on a branch, open a PR, and every PR that changes behaviour must update the relevant doc in the same PR (CI-enforced honour system; see CLAUDE.md). The audit + migration history and the original implementation plan are held with the committee records ("NNT Auth Service Plan").
