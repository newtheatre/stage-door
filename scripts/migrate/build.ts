...
for (const [email, group] of byLowerEmail) {
  ...
  prosUsers.push({
    ...
    // toMs first: the two source formats differ at index 10 (' ' vs 'T'), so
    // a raw sort orders every ISO string after every SQLite one.
    last_login: group.map(u => u.last_login).filter(Boolean)
      .sort((a, b) => toMs(a)! - toMs(b)!).at(-1) ?? null,
...
const prosFixes: string[] = [
  ...
  '-- proscenium DB at migration time: every column referencing users.id is',
  '-- re-pointed to the surviving row before the loser rows are deleted.',
  'BEGIN TRANSACTION;',
]

// Must match proscenium's own merge hook (server/utils/mergeUser.ts): a
// missed column either aborts the DELETE or nulls an attribution silently.
const PROSCENIUM_USER_REFS = [
  { table: 'reservations', column: 'user_id' },
  { table: 'passes', column: 'user_id' },
  { table: 'passes', column: 'issued_by_user_id' },
  { table: 'pass_admissions', column: 'redeemed_by_user_id' },
]

for (const fold of prosFolds) {
  for (const loser of fold.losers) {
    for (const { table, column } of PROSCENIUM_USER_REFS) {
      prosFixes.push(`UPDATE ${table} SET ${column} = ${sqlString(fold.winner)} WHERE ${column} = ${sqlString(loser)};`)
    }
    prosFixes.push(`DELETE FROM users WHERE id = ${sqlString(loser)};`)
  }
}
prosFixes.push('COMMIT;')
